...
export default function AdminPanel({ open, onClose, onChanged }: { open: boolean; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (!r.ok) return setError("Enregistrement impossible. Vérifiez la configuration Supabase.");
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Administration Holonews"><div className="admin-modal"><button className="modal-close" onClick={onClose} aria-label="Fermer"><X /></button>
    {!authenticated ? <form className="login-form" onSubmit={login}><span className="eyebrow">Accès restreint</span><h2>Console Je’Daii</h2><label>Mot de passe<input autoFocus type="password" value={password} onChange={(e) => setPassword(e.target.value)} required /></label>{error && <p className="form-error">{error}</p>}<button className="primary-action" type="submit">S’identifier</button></form> : <>
